...
int nmea_parser_init(nmeaPARSER *parser)
{
    ...
    if(0 == (parser->buffer = pvPortMalloc(buff_size)))
...
int nmea_parser_real_push(nmeaPARSER *parser, const char *buff, int buff_sz)
{
    ...
    for(;;node = 0)
    {
        ...
        else if(crc >= 0)
        {
            ...
            if(0 == (node = pvPortMalloc(sizeof(nmeaParserNODE))))
                goto mem_fail;
            ...
            {
            case GPGGA:
                if(0 == (node->pack = pvPortMalloc(sizeof(nmeaGPGGA))))
                    goto mem_fail;
                node->packType = GPGGA;
                if(!nmea_parse_GPGGA(
                    (const char *)parser->buffer + nparsed,
                ...
                break;
            case GPGSA:
                if(0 == (node->pack = pvPortMalloc(sizeof(nmeaGPGSA))))
                    goto mem_fail;
                node->packType = GPGSA;
                if(!nmea_parse_GPGSA(
                    (const char *)parser->buffer + nparsed,
                ...
                break;
            case GPGSV:
                if(0 == (node->pack = pvPortMalloc(sizeof(nmeaGPGSV))))
                    goto mem_fail;
                node->packType = GPGSV;
                if(!nmea_parse_GPGSV(
                    (const char *)parser->buffer + nparsed,
                ...
                break;
            case GPRMC:
                if(0 == (node->pack = pvPortMalloc(sizeof(nmeaGPRMC))))
                  goto mem_fail;
                ...
                break;
            case GPVTG:
                if(0 == (node->pack = pvPortMalloc(sizeof(nmeaGPVTG))))
                    goto mem_fail;
                node->packType = GPVTG;
                if(!nmea_parse_GPVTG(
                    (const char *)parser->buffer + nparsed,
                ...
                break;
            case GNGGA:
                if(0 == (node->pack = pvPortMalloc(sizeof(nmeaGNGGA))))
                    goto mem_fail;
                node->packType = GNGGA;
                if(!nmea_parse_GNGGA(
                    (const char *)parser->buffer + nparsed,
                ...
                break;
            case GNRMC:
                if(0 == (node->pack = pvPortMalloc(sizeof(nmeaGNRMC))))
                  goto mem_fail;
            ...
            case GNVTG:
                if(0 == (node->pack = pvPortMalloc(sizeof(nmeaGNVTG))))
                    goto mem_fail;
                node->packType = GNVTG;
                if(!nmea_parse_GNVTG(
                    (const char *)parser->buffer + nparsed,
                ...
                break;
            case GNZDA:
                if(0 == (node->pack = pvPortMalloc(sizeof(nmeaGNZDA))))
                    goto mem_fail;
                node->packType = GNZDA;
                if(!nmea_parse_GNZDA(
                    (const char *)parser->buffer + nparsed,
                ...
                break;
            case GNGLL:
                if(0 == (node->pack = pvPortMalloc(sizeof(nmeaGNGLL))))
                  goto mem_fail;
                ...
                break; 
            case BDGSV:
                if(0 == (node->pack = pvPortMalloc(sizeof(nmeaBDGSV))))
                  goto mem_fail;
                ...
                break; 
            case BDGSA:
                if(0 == (node->pack = pvPortMalloc(sizeof(nmeaBDGSA))))
                  goto mem_fail;
            ...
            case GPTXT:
                if(0 == (node->pack = pvPortMalloc(sizeof(nmeaGPTXT))))
                  goto mem_fail;
